...
const rows = (lines: { description: string; amount: string }[]) =>
  lines
    .map(
      (l) =>
        `<tr><td style="padding:4px 0;color:#b9b5ab">${l.description}</td><td style="padding:4px 0;text-align:right;color:#f5f2ec">${l.amount}</td></tr>`,
    )
    .join("");
...
export function customerConfirmation(
  ...
  const html = shell(
    `<h1 style="font-size:24px;margin:0 0 8px">¡Reserva confirmada!</h1>
     <p style="color:#b9b5ab;margin:0 0 16px">${v.name ? `Hola ${v.name}, ` : ""}tu sesión quedó reservada.</p>
     <p style="margin:0 0 4px"><strong>${v.when}</strong></p>
     <p style="color:#b9b5ab;margin:0 0 16px">${ctx.address}</p>
...
export function ownerNotification(
  ...
  const html = shell(
    `<h1 style="font-size:22px;margin:0 0 8px">Nueva reserva pagada</h1>
     <p style="margin:0 0 4px"><strong>${v.when}</strong></p>
     <p style="color:#b9b5ab;margin:0 0 16px">${v.name ?? "Cliente"} · ${v.email ?? "sin email"}</p>
     <table style="width:100%;border-top:1px solid #1e1d1a;border-bottom:1px solid #1e1d1a;margin:8px 0">${rows(v.lines)}</table>
