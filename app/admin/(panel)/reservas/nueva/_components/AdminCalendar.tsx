import { DateTime } from "luxon";
import { monthGrid, type DayStatus } from "@/src/domain/scheduling/month-availability";
import { Skeleton } from "@/components/admin/ui/Skeleton";

/**
 * Calendario del admin (lunes primero). A diferencia del público, los días
 * pasados/cerrados/llenos SON seleccionables: el staff necesita ver el contexto
 * del día, y una cortesía puede registrarse en el pasado o fuera de horario.
 */
interface AdminCalendarProps {
  month: string; // "YYYY-MM" visible
  today: string; // "YYYY-MM-DD" local
  maxDate: string; // horizonte
  selected: string | null;
  dayStatus: Record<string, DayStatus>;
  loading?: boolean;
  onSelect: (date: string) => void;
  onMonth: (month: string) => void;
}

const WEEKDAYS = ["Lu", "Ma", "Mi", "Ju", "Vi", "Sá", "Do"];
const MONTHS = [
  "Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio",
  "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre",
];

const STATUS_LABEL: Record<DayStatus, string> = {
  closed: ", cerrado",
  full: ", lleno",
  low: ", pocos horarios",
  open: "",
};

export function AdminCalendar({
  month, today, maxDate, selected, dayStatus, loading = false, onSelect, onMonth,
}: AdminCalendarProps) {
  const grid = monthGrid(month);
  const [y, m] = month.split("-").map(Number);
  const minMonth = today.slice(0, 7);
  const maxMonth = maxDate.slice(0, 7);
  const shift = (delta: number) =>
    onMonth(DateTime.fromISO(`${month}-01`).plus({ months: delta }).toFormat("yyyy-MM"));

  const navCls =
    "flex size-9 items-center justify-center border hairline text-bone transition-colors outline-none " +
    "hover:border-gold hover:text-gold focus-visible:ring-1 focus-visible:ring-gold disabled:opacity-25";

  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <button type="button" aria-label="Mes anterior" disabled={month <= minMonth} onClick={() => shift(-1)} className={navCls}>
          ‹
        </button>
        <span className="font-display text-lg text-bone">{MONTHS[m - 1]} {y}</span>
        <button type="button" aria-label="Mes siguiente" disabled={month >= maxMonth} onClick={() => shift(1)} className={navCls}>
          ›
        </button>
      </div>

      <div className="mb-2 grid grid-cols-7 gap-1">
        {WEEKDAYS.map((d) => (
          <div key={d} className="label-sm py-1 text-center text-bone-mute">{d}</div>
        ))}
      </div>

      <div className="grid grid-cols-7 gap-1">
        {loading
          ? Array.from({ length: 42 }).map((_, i) => <Skeleton key={i} className="aspect-square" />)
          : grid.flat().map((cell) => {
              const status = dayStatus[cell.date] ?? "open";
              const isPast = cell.date < today;
              const disabled = !cell.inMonth || cell.date > maxDate;
              const isSelected = cell.date === selected;
              const isToday = cell.date === today;
              const dim = isPast || status === "closed" || status === "full";

              return (
                <button
                  key={cell.date}
                  type="button"
                  disabled={disabled}
                  aria-pressed={isSelected}
                  aria-label={
                    DateTime.fromISO(cell.date).setLocale("es").toFormat("d 'de' MMMM yyyy") +
                    (isPast ? ", pasado" : STATUS_LABEL[status])
                  }
                  onClick={() => onSelect(cell.date)}
                  className={[
                    "relative flex aspect-square items-center justify-center font-mono text-sm transition-colors",
                    "outline-none focus-visible:ring-1 focus-visible:ring-gold",
                    !cell.inMonth ? "text-bone-mute/30" : "",
                    isSelected ? "bg-gold text-ink" : "",
                    !isSelected && cell.inMonth && dim ? "text-bone-mute/40 hover:bg-ink-soft" : "",
                    !isSelected && cell.inMonth && !dim ? "text-bone hover:bg-ink-soft hover:text-gold" : "",
                    isToday && !isSelected ? "ring-1 ring-inset ring-gold/50" : "",
                  ].join(" ")}
                >
                  {Number(cell.date.slice(8, 10))}
                  {cell.inMonth && !isSelected && !isPast && status === "low" && (
                    <span aria-hidden className="absolute bottom-1 left-1/2 size-1 -translate-x-1/2 rounded-full bg-sirena" />
                  )}
                  {cell.inMonth && !isSelected && !isPast && status === "full" && (
                    <span aria-hidden className="absolute bottom-1 left-1/2 size-1 -translate-x-1/2 rounded-full bg-bone-mute" />
                  )}
                </button>
              );
            })}
      </div>

      {!loading && (
        <p className="label-sm mt-4 flex items-center gap-3 text-bone-mute">
          <span className="flex items-center gap-1.5">
            <span aria-hidden className="size-1 rounded-full bg-sirena" /> Pocos horarios
          </span>
          <span className="flex items-center gap-1.5">
            <span aria-hidden className="size-1 rounded-full bg-bone-mute" /> Lleno
          </span>
        </p>
      )}
    </div>
  );
}
