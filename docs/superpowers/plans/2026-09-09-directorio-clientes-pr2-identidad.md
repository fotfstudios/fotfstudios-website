...
Consolidating four copies of `normalizePhone` into one Chile-aware helper, and routing `/cuenta/perfil` through `update_customer_contact`, changes behaviour beyond a pure refactor. All of it is intentional; all of it goes in the PR body (Task 9):
...
5. `/api/bookings` ahora recorta el nombre del cliente a 80 caracteres (el CHECK
   `customers_name_len`).
6. **Lo más importante:** guardar el perfil ahora pasa por `update_customer_contact`, que además
